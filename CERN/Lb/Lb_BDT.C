...
int Lb_BDT(TString myMethodList = ""){
  ...
  for (Int_t i = 0; i < nPtBins; i++) { //master loop over all ptbins
    TFile* inputSignal(0);
    TString fnamesig = Form("/home/mjongerh/LbcutMid_data/Trees/Lb_binned_signal_Pt%.1f.root", ptBins[i]); //Signal Input folder      /home/mjongerh/Lb_data/Trees/
    ...
    TString fnamebkg = Form("/home/mjongerh/LbcutMid_data/Trees/Lb_binned_bkg_Pt%.1f.root", ptBins[i]); //Background Input folder
    ...
    TString outfileDir = Form("/home/mjongerh/Lb_data/outputmidcut/Pt%.1f", ptBins[i]);
