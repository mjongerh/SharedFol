...
int Lb_BDT(TString myMethodList = ""){
  ...
  for (Int_t i = 5; i < 6; i++) { //nPtBins //master loop over all ptbins
    TFile* inputSignal(0);
    ...
    TString fnamebkg = Form("/home/mjongerh/LbcutStrict_data/Trees/Lb_binned_bkg_Pt%.1f.root", ptBins[i]); //Background Input folder
